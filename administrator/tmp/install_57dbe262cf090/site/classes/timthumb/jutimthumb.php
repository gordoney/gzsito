...
if (!class_exists('jutimthumb'))
{
	...
	if (file_exists(dirname(__FILE__) . '/jutimthumb-config.php'))
	{
		require_once('jutimthumb-config.php');
	}
	if (!defined('DEBUG_ON'))
	{
	...
	*/
	...
	class jutimthumb
	{
		...
		//We need to create new 2 varibles, default value = CONST, because these variables can be changed if output is not specified(In this case we use php temp dir)
		protected $file_cache_directory = FILE_CACHE_DIRECTORY;
		protected $file_cache_time_between_cleans = FILE_CACHE_TIME_BETWEEN_CLEANS;

		public function __construct($param_array)
		{
			//Set param_array
			$this->param_array = $param_array;
			if (!isset($this->param_array['display_image']))
			{
				$this->param_array['display_image'] = false;
			}

			/*param_array['output'] =
				-> TIMTHUMB_CACHE: Use timthumb cache dir to render TMP image to timthumb cache(to display tmp resize/crop image)
				-> Specified image path: Use sys_get_temp_dir to render image to specified path(resize/crop task)
				-> Not set or false: Use sys_get_temp_dir to render TMP image to php tmp dir(if we want to use tmp image to process an other task)
			*/

			//If set output = 'TIMTHUMB_CACHE' -> use default timthumb cache as output dir, and clean cache as timthumb config(do nothing)
			if (strtoupper($this->param_array['output']) == "TIMTHUMB_CACHE")
			{
				$this->param_array['output'] = '';
				//$file_cache_directory = FILE_CACHE_DIRECTORY;
				//If set output = specified image path OR not set output -> use sys_get_temp_dir to render tmp image, don't need to clean timthumb cache dir(FILE_CACHE_DIRECTORY)
			}
			else
			{
				//If get writable issue with sys_get_temp_dir, we can ignore this case to use timthumb cache dir
				//We will use sys_get_temp_dir to render tmp image
				$this->file_cache_directory = '';
				//We don't use timthumb cache dir, so don't need to clean timthumb cache dir in this case
				$this->file_cache_time_between_cleans = -1;
			}

			global $ALLOWED_SITES;
			$this->startTime = microtime(true);
			date_default_timezone_set('UTC');
			$this->debug(1, "<=============== STARTING ===============>");
			$this->debug(1, "Starting new request from " . $this->getIP() . " to " . $_SERVER['REQUEST_URI']);
			$this->calcDocRoot();
			//On windows systems I'm assuming fileinode returns an empty string or a number that doesn't change. Check this.
			$this->salt = @filemtime(__FILE__) . '-' . @fileinode(__FILE__);
			$this->debug(3, "Salt is: " . $this->salt);
			if ($this->file_cache_directory)
			{
				if (!is_dir($this->file_cache_directory))
				{
					//@mkdir($this->file_cache_directory);
					//Use joomla JFolder to create recursive directories
					JFolder::create($this->file_cache_directory);
					if (!is_dir($this->file_cache_directory))
					{
				...
				$this->cacheDirectory = $this->file_cache_directory;
			...
			$cachePrefix = '_'; //Modified - Don't need to use int/ext
			$querystring = http_build_query($this->param_array); //Re-produce $_SERVER ['QUERY_STRING']
			if ($this->isURL)
			{
				$arr = explode('&', $querystring);
				asort($arr);
				if ($this->param_array['output'])
				{
					$this->cachefile = $this->param_array['output'];
				}
				else
				{
					$this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . pathinfo($this->src, PATHINFO_FILENAME) . $cachePrefix . md5($this->salt . implode('', $arr) . $this->fileCacheVersion) . "_" . $this->param('w') . "x" . $this->param('h') . FILE_CACHE_SUFFIX . "." . pathinfo($this->src, PATHINFO_EXTENSION);
				}
			}
			else
			{
				...
				else
				{
					...
					$this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . pathinfo($this->src, PATHINFO_FILENAME) . $cachePrefix . md5($this->salt . $this->localImageMTime . $querystring . $this->fileCacheVersion) . "_" . $this->param('w') . "x" . $this->param('h') . FILE_CACHE_SUFFIX . "." . pathinfo($this->src, PATHINFO_EXTENSION);
				}
			}
		...
		public function start()
		{
			...
			//If NOT set output, or output = timthumb cache -> try to return cache file to reduce load
			if (!$this->param_array['output'] || strtoupper($this->param_array['output']) == "TIMTHUMB_CACHE")
			{
				if ($this->tryBrowserCache())
				{
					if (file_exists($this->cachefile))
					{
						if ($this->param_array['display_image'])
						{
							$this->serveImg($this->cachefile);
							exit(0);
						}
						else
						{
			...
			//If set output => resize image to specified path, NO cache
		...
		protected function error($err)
		{
			...
			$this->errors[] = $err;
			//Modify: Allow to config to display error in Joomla
			if (DISPLAY_JERROR_MESSAGES)
			{
				JFactory::getApplication()->enqueueMessage("TimThumb: $err", 'error');
			}
		...
		{
			if ($this->file_cache_time_between_cleans < 0)
			{
				return;
			}
			...
			if (@filemtime($lastCleanFile) < (time() - $this->file_cache_time_between_cleans))
			{ //Cache was last cleaned more than 1 day ago
				$this->debug(1, "Cache was last cleaned more than " . $this->file_cache_time_between_cleans . " seconds ago. Cleaning now.");
				// Very slight race condition here, but worst case we'll have 2 or 3 servers cleaning the cache simultaneously once a day.
				if (!touch($lastCleanFile))
				{
					$this->error("Could not create cache clean timestamp file.");
				}
				$files = glob($this->cacheDirectory . '/*' . FILE_CACHE_SUFFIX . '.{jpg,gif,png}', GLOB_BRACE);
				if ($files)
				{
			...
			{
				$this->debug(3, "Cache was cleaned less than " . $this->file_cache_time_between_cleans . " seconds ago so no cleaning needed.");
			}
		...
		protected function processImageAndWriteToCache($localImage)
		{
			...
			//file_put_contents($tempfile4, $this->filePrependSecurityBlock . $imgType . ' ?' . '>'); //6 extra bytes, first 3 being image type //Disable this line allow direct view image
			...
			if (flock($fh, LOCK_EX))
			{
				...
				//Modified: Use JFile::move instead rename(This can raise message: Rename failed)
				//JFile::move($tempfile4, $this->cachefile);
		...
		protected function serveExternalImage()
		{
			...
			$tempfile = tempnam($this->cacheDirectory, 'jutimthumb');
		...
		{
			/** Modified - Don't need to display image. Everything is done here! **/
		...
		protected function param($property, $default = '')
		{
			...
			if (isset ($param_array[$property]))
			{
				return $param_array[$property];
			}
		...
		protected function debug($level, $msg)
		{
			if (DEBUG_ON && $level <= DEBUG_LEVEL)
			{
				...
				//Modify: If turn on debug, we show debug messages in Joomla
				JFactory::getApplication()->enqueueMessage("TimThumb Debug line " . __LINE__ . " [$execTime : $tick]: $msg", 'message');
			}
		}
		...
		protected function getURL($url, $tempfile)
		{
			...
			if (function_exists('curl_init'))
			{
				...
				curl_setopt($curl, CURLOPT_WRITEFUNCTION, 'jutimthumb::curlWrite');
