...
if (is_array($values))
{
	...
	foreach ($values AS $value)
	{
		...
		{
			$item = "<a href =\"" . JRoute::_("index.php?option=com_jucomment&view=searchby&field_id=" . $this->id . "&value=" . JUCommentFrontHelper::UrlEncode($value), false) . "\">" . $value . "</a>";
		}
		else
		{
			$item = $value;
...
else
{
		...
		if ($values)
		{
			...
			foreach ($values AS $value)
			{
				if ($value)
				{
					...
					$items[] = "<span><a href =\"" . JRoute::_("index.php?option=com_jucomment&view=searchby&field_id=" . $this->id . "&value=" . JUCommentFrontHelper::UrlEncode($value)) . "\">" . $value . "</a></span>";
				}
			}
	...
	if ($this->params->get("parse_plugin", 0))
	{
		...
		$html_values = JHtml::_('content.prepare', $html_values, $params, 'com_jucomment.field');
	}
	$html = '';
