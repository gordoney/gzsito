...
<form action="" method="post" name="jucm-form-comments full-view" id="jucm-form-comments">
	<div class="clearfix">
		...
		<div class="filter-sort pull-left">
			...
			<select name="filter_order_Dir" class="input-medium sort-direction" onchange="this.form.submit()">
			...
			?>
			<a href="<?php echo JRoute::_('index.php?option=com_jucomment&view=usercomments', false); ?>" class="btn btn-default btn-sm"
								...
								<div class="jucm-metadata clearfix">
									...
									?>
										<h4 class="comment-title" itemprop="name">
											<?php $return = base64_encode('index.php?option=com_jucomment&view=usercomments&layout=full'); ?>
									...
									<ul class="dropdown-menu">
										...
										if($row->approved == 0 && $system->permission->allow('approve', $row))
										{
											...
											<li>
												...
												{
													$url = 'index.php?option=com_jucomment&task=usercomments.approve&cid=' . $row->id .
														'&'. JSession::getFormToken() . '=1&return=' . base64_encode($uri);
												?>
													<a href="<?php echo $url; ?>" title="<?php echo JText::_('COM_JUCOMMENT_APPROVE_COMMENT'); ?>">
														<i class="icon-unpublish"></i> <?php echo JText::_('COM_JUCOMMENT_APPROVE_COMMENT'); ?>
													</a>
												<?php
												}
												?>
										...
										if($system->permission->allow('edit_state', $row))
										{
											...
											<li>
												...
												{
													$url = 'index.php?option=com_jucomment&task=usercomments.unpublish&cid=' . $row->id
														. '&' . JSession::getFormToken() . '=1' . '&return=' . base64_encode($uri);
													?>
												...
												<?php
													...
													$url = 'index.php?option=com_jucomment&task=usercomments.publish&cid=' . $row->id .
														'&'. JSession::getFormToken() . '=1' . '&return=' . base64_encode($uri);
												?>
										...
										{
											$url = 'index.php?option=com_jucomment&task=usercomments.delete&cid=' . $row->id
												. '&' . JSession::getFormToken() . '=1' . '&return=' . base64_encode($uri);
											?>
											<li>
