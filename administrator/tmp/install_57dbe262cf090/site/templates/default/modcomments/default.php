...
<?php if (!$view->items)
{
	...
	<div class="alert alert-block">
		...
		<?php echo JText::_('COM_JUCOMMENT_NO_COMMENT'); ?>
	</div>
<?php
} ?>
...
<form name="jucm-form-comments" id="jucm-form-comments" class="jucm-form" method="post" action="">
	...
	<div class="jucm-filter-sort clearfix">
		...
		<div class="filter-sort pull-left">
			...
			<select name="filter_mod_type" class="jucm-mod-type" onchange="this.form.submit()">
				<?php echo JHtml::_('select.options', $view->getPermissions(), 'value', 'text', $view->state->get('filter.permission', '')); ?>
			</select>
			<select name="action" class="jucm-action" onchange="this.form.submit()">
				<?php echo JHtml::_('select.options', $view->getActions(), 'value', 'text', ''); ?>
			</select>
			<select name="filter_order" class="jucm-order-sort" onchange="this.form.submit()">
				<?php echo JHtml::_('select.options', $view->getSortFields(), 'value', 'text', $view->state->get('list.ordering', '')); ?>
			</select>
			<select name="filter_order_Dir" class="jucm-order-dir" onchange="this.form.submit()">
			...
			?>
			<a href="<?php echo JRoute::_('index.php?option=com_jucomment&view=modcomments&layout=full', false); ?>" class="btn btn-default btn-sm"
				...
				<tr class="<?php echo $class; ?>">
					...
					<td>
						...
						<a class="privew-comment fancybox.ajax" href="<?php echo JRoute::_('index.php?option=com_jucomment&view=comment&layout=preview&id=' . $row->id . '&tmpl=component'); ?>" data-facybox-type="ajax">
							<i class="fa fa-search-plus"></i> <?php echo JText::_('COM_JUCOMMENT_PREVIEW'); ?>
						</a>
					</td>
					<td>
						<?php
